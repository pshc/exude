...
macro_rules! gfx_impl_struct_meta {
    ($(#[$attr:meta])* impl_struct_meta $runtime_format:ty : $compile_format:path = $root:ident {
        ...
        unsafe impl $crate::gfx::traits::Pod for $root {}

        impl $crate::gfx::pso::buffer::Structure<$runtime_format> for $root {
            fn query(name: &str) -> Option<$crate::gfx::pso::buffer::Element<$runtime_format>> {
                use std::mem::{size_of, transmute};
                use $crate::gfx::pso::buffer::{Element, ElemOffset};
...
macro_rules! gfx_vertex_struct_meta {
    ($(#[$attr:meta])* vertex_struct_meta $root:ident {
        ...
        $crate::gfx::format::Format : $crate::gfx::format::Formatted =
        $root {
            $( $field: $ty = $name, )*
        }
    })
...
macro_rules! gfx_constant_struct_meta {
    ($(#[$attr:meta])* constant_struct_meta $root:ident {
        ...
        $(#[$attr])* impl_struct_meta
        $crate::gfx::shade::ConstFormat : $crate::gfx::shade::Formatted =
        $root {
            $( $field: $ty = $name, )*
        }
    })
